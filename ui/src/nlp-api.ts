...
export interface IChoreClient {
    ...
    deleteChore(chore: HomeChoreDto): Promise<BoolResponse>;
}
...
export class ChoreClient extends NlpBaseClient implements IChoreClient {
    ...
    deleteChore(chore: HomeChoreDto): Promise<BoolResponse> {
        let url_ = this.baseUrl + "/api/Chore/delete-chore";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.transformOptions(options_).then(transformedOptions_ => {
            ...
            return this.transformResult(url_, _response, (_response: Response) => this.processDeleteChore(_response));
        });
    }

    protected processDeleteChore(response: Response): Promise<BoolResponse> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 200) {
            return response.text().then((_responseText) => {
