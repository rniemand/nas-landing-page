...
export class UserLinksClient {
    ...
    getCategories(): Observable<string[]> {
        let url_ = this.baseUrl + "/api/UserLinks/categories";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processGetCategories(response: HttpResponseBase): Observable<string[]> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
    ...
    getImage(image: string): Observable<FileResponse | null> {
        ...
        if (this.badges) {
            ...
            for (let key in this.badges) {
                if (this.badges.hasOwnProperty(key))
                    (<any>data["badges"])[key] = (<any>this.badges)[key];
            }
        }
        return data;
    }
